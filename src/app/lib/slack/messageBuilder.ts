 ...
 */
export function buildEarthquakeNotificationMessage(
  ...
  const prefectureText =
    earthquake.prefectureObservations && earthquake.prefectureObservations.length > 0
 ...
 * ボタンの色をSlackのスタイルに変換
 */
function getButtonStyle(color: string): "primary" | "danger" | undefined {
  if (color.toLowerCase().includes("blue")) return "primary";
  if (color.toLowerCase().includes("red")) return "danger";
  return undefined;
}
 ...
 */
export function buildUpdatedMessageWithStats(
  originalMessage: any,
  stats: { [departmentId: string]: number }
) {
  const blocks = [...originalMessage.blocks];

  // 既存の統計セクションを削除（あれば）
  const statsIndex = blocks.findIndex(
    (block: any) => block.block_id === "safety_stats"
  );
  if (statsIndex !== -1) {
