// src/app/api/admin/cronjob-api-keys/route.ts
...
/**
 * API Key一覧取得
 * GET /api/admin/cronjob-api-keys
 */
export async function GET(request: NextRequest) {
  try {
    const apiKeys = await prisma.cronJobApiKey.findMany({
...
/**
 * API Key登録
 * POST /api/admin/cronjob-api-keys
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // 既存のAPIキーと重複チェック
    const existingKeys = await prisma.cronJobApiKey.findMany();
    ...
    const newKey = await prisma.cronJobApiKey.create({
      data: {
        apiKey: encryptedJson,
        description: description || "cron-job.org APIキー",
        isActive: true,
      },
    });
