...
import { TsunamiWarning, TsunamiArea, TsunamiHeight, TSUNAMI_COASTAL_AREAS } from '../types/TsunamiTypes';
import { WebSocketMessage } from '../types/WebSocketTypes';

// 津波情報をWebSocketメッセージから抽出する関数
export const processTsunamiMessage = (message: WebSocketMessage): TsunamiWarning | null => {
  try {
    ...
    // 津波関連メッセージかチェック
    if (!message.classification?.includes('tsunami') && 
    ...
    let warningType: 'major_warning' | 'warning' | 'advisory' | 'forecast' = 'forecast';
    ...
    if (infoKind.includes('大津波警報') || title.includes('大津波警報')) {
      ...
      warningType = 'warning';
    } else if (infoKind.includes('津波注意報') || title.includes('津波注意報')) {
    ...
    // 解除情報の判定
    const isCancel = headline.includes('解除') || headline.includes('取消') || title.includes('解除');
    
    // 津波到達予想時刻
    const expectedArrival = body.tsunami?.expectedArrival || body.tsunami?.arrivalTime;
    ...
    if (Array.isArray(tsunamiAreas)) {
      tsunamiAreas.forEach((area: any) => {
        ...
        const prefecture = area.prefecture || extractPrefectureFromName(areaName);
...
// 沿岸部座標の取得
const getCoastalCoordinates = (areaCode: string, areaName: string): [number, number][] | undefined => {
  // コードで直接検索
  if (areaCode && TSUNAMI_COASTAL_AREAS[areaCode as keyof typeof TSUNAMI_COASTAL_AREAS]) {
    return TSUNAMI_COASTAL_AREAS[areaCode as keyof typeof TSUNAMI_COASTAL_AREAS].coordinates;
  }
  ...
  console.warn(`No coordinates found for tsunami area: ${areaCode} - ${areaName}`);
  return undefined;
};
